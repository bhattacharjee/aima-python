...
def SimpleReflexProgram(weighted_rand_sel=False, use_inference=False):
    ...
    def program(percepts):
        ...
        # Remove positions that we know are hawks
        temp_positions = Utils.trim_candidate_if_hawk(kb, shreik_heard, temp_positions, ag_location, mt_dimensions)
...
class SnakeKnowledgeBaseToDetectHawk(object):
    ...
    def ask_if_location_definitely_hawk(self, shreik_heard, self_loc, new_loc, dimensions):
        ...
        if n_not_hawk == (len(adj_not_hawk) - 1):
            for nothawk, location in zip(adj_not_hawk, adj):
                if location == new_loc:
                    return not nothawk
        return False
...
def process():
    ...
    RunAgentAlgorithm(SimpleReflexProgram(weighted_rand_sel=False, use_inference=True), smallHawkTestMaze);
